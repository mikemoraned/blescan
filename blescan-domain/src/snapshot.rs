...
#[derive(Serialize, TS, PartialEq, Debug, Default, Clone)]
...
impl Snapshot {
    ...
    pub fn compared_to(
        ...
        let previous_rssi: HashMap<Signature, i16> = previous
            .0
            .iter()
            .map(|d| (d.signature.clone(), d.rssi))
            .collect();
        self.0
            .iter()
            .map(|d| {
...
mod test {
    ...
    use crate::{
        ...
        signature::Signature,
        snapshot::{Comparison, RssiComparison},
    };
    ...
    fn order_by_age_oldest_last() {
        let initial_order = Snapshot(vec![
            DeviceState::new(
                Utc.timestamp_opt(1, 0).unwrap(),
                Signature::Named("1".to_string()),
                -1,
            ),
            DeviceState::new(
                Utc.timestamp_opt(2, 0).unwrap(),
                Signature::Named("2".to_string()),
                -1,
            ),
            DeviceState::new(
                Utc.timestamp_opt(3, 0).unwrap(),
                Signature::Named("3".to_string()),
                -1,
            ),
        ]);
        let expected_order = Snapshot(vec![
            DeviceState::new(
                Utc.timestamp_opt(3, 0).unwrap(),
                Signature::Named("3".to_string()),
                -1,
    ...
    fn relative_age() {
        ...
        let actual_comparisons = snapshot.compared_to(now, &Snapshot::default());
    ...
    fn relative_volume() {
        ...
        let actual_comparisons = current.compared_to(now, &previous);

        assert_eq!(
            just_rssi(&actual_comparisons),
            just_rssi(&expected_comparisons)
        );
        assert_eq!(actual_comparisons, expected_comparisons);
    }
}
